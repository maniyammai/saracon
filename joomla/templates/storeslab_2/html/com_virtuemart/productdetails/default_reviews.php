			...
			foreach ($this->rating_reviews as $review) {
				...
				if ($i == $ratingsShow && !$showall) {
					...
					if ($reviews_published >= $ratingsShow) {
						$attribute = array('class'=> 'details dboxname', 'title'=> JText::_ ('COM_VIRTUEMART_MORE_REVIEWS'));
...
//]]>
			...
			if ($this->showRating) {
				if ($this->allowRating && $review_editable) {
					?>
					<h4 class="dboxname box7br"><?php echo JText::_ ('COM_VIRTUEMART_WRITE_REVIEW')  ?><span><?php echo JText::_ ('COM_VIRTUEMART_WRITE_FIRST_REVIEW') ?></span></h4>
			...
			if ($review_editable) {
				...
				<br /><br />
				<input class="button boxcart box4br_radius boxcart_shadow" type="submit" onclick="return( check_reviewform());" name="submit_review" title="<?php echo JText::_ ('COM_VIRTUEMART_REVIEW_SUBMIT')  ?>" value="<?php echo JText::_ ('COM_VIRTUEMART_REVIEW_SUBMIT')  ?>"/>
				<?php
			} else {
